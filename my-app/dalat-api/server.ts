...
async function ensureUserColumns() {
  const statements = [
    ...
    "ALTER TABLE users ADD COLUMN vipTier VARCHAR(20) DEFAULT NULL",
    "ALTER TABLE users ADD COLUMN quarterlySpending DECIMAL(12,2) DEFAULT 0",
    "ALTER TABLE users ADD COLUMN rewardPoints DECIMAL(12,2) DEFAULT 0",
    "ALTER TABLE users ADD COLUMN vipQuarterKey VARCHAR(20) DEFAULT NULL",
    "ALTER TABLE users ADD COLUMN vipTierUpdatedAt DATETIME DEFAULT NULL",
  ];
...
async function ensureInventoryTables() {
  const statements = [
    ...
    "ALTER TABLE Products ADD COLUMN unit VARCHAR(20) DEFAULT 'kg'",
...
async function ensureOrderColumns() {
  try {
    await pool.query("ALTER TABLE orders MODIFY COLUMN userId INT NULL");
  } catch (error: unknown) {
    const message = error instanceof Error ? error.message : String(error);
...
async function startServer() {
  try {
    ...
    console.error("❌ Lỗi kết nối MySQL:", (err as any).message);
