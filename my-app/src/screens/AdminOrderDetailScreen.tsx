...
export default function AdminOrderDetailScreen() {
  ...
  const printOrder = async () => {
    ...
    const rows = (order.items ?? []).map((item: any, index: number) => `
      <tr><td>${index + 1}</td><td>${item.productName ?? ''}</td><td>${isNormalOrder ? item.quantity : `${item.kg} kg`}</td><td>${isNormalOrder ? money(item.price) : '&nbsp;'}</td><td>${isNormalOrder ? money(Number(item.price) * Number(item.quantity)) : '&nbsp;'}</td></tr>
