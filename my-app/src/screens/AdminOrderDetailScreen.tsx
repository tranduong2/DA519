...
export default function AdminOrderDetailScreen() {
  ...
  const printOrder = async () => {
    ...
    const rows = (order.items ?? []).map((item: any, index: number) => `
      <tr><td>${index + 1}</td><td>${item.productName ?? ''}</td><td>${isNormalOrder ? item.quantity : `${item.kg} kg`}</td><td>${money(isNormalOrder ? item.price : item.pricePerKg)}</td><td>${money(isNormalOrder ? Number(item.price) * Number(item.quantity) : item.subtotal)}</td></tr>
      ${item.note ? `<tr class="note"><td></td><td colspan="4">Ghi chú: ${item.note}</td></tr>` : ''}
    `).join('');
