...
export default function StoreInvoiceTotalsScreen() {
  ...
  const dailyTotals = Object.values((periodOrders as any[]).reduce((result: Record<string, any>, order: any) => {
    const date = new Date(order.createdAt).toLocaleDateString('vi-VN');
    if (!result[date]) result[date] = { date, count: 0, total: 0 };
    result[date].count += 1; result[date].total += Number(order.totalPrice) || 0;
    return result;
  }, {}));
  const periodTotal = dailyTotals.reduce((sum: number, day: any) => sum + day.total, 0);
  const printPeriod = async () => {
    ...
    if (Platform.OS === 'web') {
      ...
      if (!popup) return;
      const rows = dailyTotals.map((day: any, index: number) => `<tr><td>${index + 1}</td><td>${day.date}</td><td>${day.count}</td><td>${money(day.total)}</td></tr>`).join('');
      popup.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>${title}</title><style>@page{size:A4;margin:15mm}body{font-family:Arial;color:#263238}h1{color:#1b5e20}table{width:100%;border-collapse:collapse}th,td{border:1px solid #607d8b;padding:9px}th{background:#1b5e20;color:#fff}.right{text-align:right}.total{font-size:20px;font-weight:bold;text-align:right;margin-top:18px}</style></head><body><h1>${title}</h1><p>Từ ${periodStart.toLocaleDateString('vi-VN')} đến ${periodEnd.toLocaleDateString('vi-VN')}</p><table><tr><th>STT</th><th>NGÀY</th><th>SỐ HÓA ĐƠN</th><th>TỔNG TIỀN</th></tr>${rows}</table><div class="total">TỔNG 20 NGÀY: ${money(periodTotal)}</div><script>window.onload=()=>window.print()<\/script></body></html>`);
      popup.document.close(); return;
    }
    await Share.share({ title, message: `${title}\n${periodStart.toLocaleDateString('vi-VN')} - ${periodEnd.toLocaleDateString('vi-VN')}\n\n${dailyTotals.map((day: any) => `${day.date}: ${day.count} hóa đơn - ${money(day.total)}`).join('\n')}\n\nTỔNG: ${money(periodTotal)}` });
  };
  return (
    <SafeAreaView style={s.page}>
      ...
      <Modal visible={!!selectedStore} transparent animationType="slide" onRequestClose={() => setSelectedStore(null)}>
        <View style={s.overlay}><View style={s.modal}>
          ...
          <View style={s.excelHead}><Text style={s.colDate}>NGÀY</Text><Text style={s.colCount}>SỐ HÓA ĐƠN</Text><Text style={s.colMoney}>TỔNG TIỀN</Text></View>
          <ScrollView style={{ maxHeight: 360 }}>{dailyTotals.map((day: any) => <View key={day.date} style={s.excelRow}><Text style={s.colDateValue}>{day.date}</Text><Text style={s.colCountValue}>{day.count}</Text><Text style={s.colMoneyValue}>{money(day.total)}</Text></View>)}{dailyTotals.length === 0 ? <Text style={s.emptyPeriod}>Chu kỳ này chưa có hóa đơn.</Text> : null}</ScrollView>
          <View style={s.periodTotal}><Text style={s.periodTotalLabel}>TỔNG CHU KỲ 20 NGÀY</Text><Text style={s.periodTotalValue}>{money(periodTotal)}</Text></View>
